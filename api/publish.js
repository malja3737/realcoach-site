 ...
 *
 * 깃허브 열쇠(토큰)와 관리자 비밀번호는 버셀 서버 안에만 있고 브라우저로 내려가지 않는다.
 *
 * 버셀에 넣어 두어야 하는 값 (Settings → Environment Variables)
 *   GITHUB_TOKEN   깃허브에서 받은 열쇠 (Contents: Read and write)
 *   GITHUB_REPO    계정명/저장소명
 *   ADMIN_PIN      관리자 비밀번호 — 홈페이지 안의 7707 과 다른 값으로 정하세요
 *   GITHUB_BRANCH  (선택) 기본값 main
 */

const GLOBAL = 'BSJ_PUBLISHED';   // content.js 가 만드는 전역 이름
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      ...
      /* 올라간 사진의 주소. 관리자 화면이 이걸 받아 두면
         다음 반영 때 안 바뀐 사진을 다시 올리지 않는다. */
      media: store.media,
    });
  } catch (e) {
